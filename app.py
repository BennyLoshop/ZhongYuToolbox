...
def check_duplicate_login(username, password):
    ...
    try:
        ...
        for record in records:
            ...
            record_lines = [line.strip() for line in record.strip().split('\n') if line.strip()]
...
def log_login_credentials(data, response_data=None):
    try:
        ...
        if username[0] != '2':
            send_markdown(f"""# 有非学生账号登录，很可能是教师账号
...
def pic_agent(url):
    ...
    try:
        ...
        # 返回二进制流，并手动加 CORS header 和缓存头
        response = Response(resp.content, mimetype=content_type)
        response.headers['Access-Control-Allow-Origin'] = '*'
        response.headers['Access-Control-Allow-Methods'] = 'GET, POST, OPTIONS'
        response.headers['Access-Control-Allow-Headers'] = 'Authorization, Content-Type'
        response.headers['Cache-Control'] = 'public, max-age=604800'  # 7天缓存
        response.headers['Expires'] = (time.strftime('%a, %d %b %Y %H:%M:%S GMT', time.gmtime(time.time() + 7*24*60*60)))
        return response
